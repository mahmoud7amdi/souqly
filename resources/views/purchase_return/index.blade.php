@extends('layouts.app')
@section('title', __('lang_v1.purchase_returns'))
@section('page_title', __('lang_v1.purchase_returns'))

@section('content')

@php $isFiltered = request()->filled('search'); @endphp

{{-- No create action in the head, and that is not an omission: purchase-return.create
     takes a {purchase}, so a return can only begin from an invoice. The empty state
...
<form method="GET" class="filter-bar">
    <div class="filter-grid">
        ...
        <div class="flex items-end gap-2">
            ...
            @if ($isFiltered)
                <a href="{{ route('purchase-return.index') }}" class="btn-secondary">
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th>{{ __('lang_v1.parent_purchase') }}</th>
        ...
        <tbody>
            @forelse ($returns as $return)
                <tr>
                    ...
                    <td class="force-ltr">
                        <a href="{{ route('purchase-return.show', $return->id) }}" class="cell-link">
                            {{ $return->ref_no }}
                        </a>
                    </td>
                    ...
                    {{-- The invoice this reverses. A link, because the next question
                         after "we returned 3 units" is always "off which purchase". --}}
                    <td class="force-ltr">
                        @if ($return->return_parent_sell)
                            <a href="{{ route('purchases.show', $return->return_parent_sell->id) }}" class="link text-xs">
            ...
            @empty
                <x-table-empty :columns="8"
                               ...
                               :text="$isFiltered ? __('lang_v1.nothing_matches_filters') : __('lang_v1.returns_start_from_a_purchase')">
                    @if ($isFiltered)
                        <a href="{{ route('purchase-return.index') }}" class="btn-secondary btn-sm">
                            {{ __('lang_v1.clear_filters') }}
                        </a>
                    @else
                        <a href="{{ route('purchases.index') }}" class="btn-secondary btn-sm">
                            <x-nav-icon name="truck" :size="4"/>
                            {{ __('lang_v1.purchases') }}
                        </a>
                    @endif
                </x-table-empty>
            @endforelse
